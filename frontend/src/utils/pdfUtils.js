...
const formatTime = (dateString) => {
    try {
        return new Date(dateString).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: false }) + " Hrs";
    } catch (e) {
        return "00:00 Hrs";
    }
};
...
export const generateTicketPDF = (booking) => {
    try {
        ...
        doc.text(`Booking Date - ${new Date(booking.bookingTime).toLocaleString()}`, 195, 31, { align: "right" });
        ...
        doc.setFontSize(10);
        doc.setTextColor(0, 0, 0);
        doc.text(`${flight.departureAirport.city} → ${flight.arrivalAirport.city}`, 20, 56);
        doc.setFont("helvetica", "normal");
        doc.setTextColor(100, 100, 100);
        ...
        const airlineColor = flight.airlineColor && flight.airlineColor.startsWith('#') ? flight.airlineColor : COLORS.primary[0];
        ...
        doc.setFontSize(12);
        doc.setFont("helvetica", "bold");
        doc.text(flight.departureAirport.code, midX - 30, startY, { align: "center" });
        doc.text(formatTime(flight.departureTime), midX - 30, startY + 6, { align: "center" });
        doc.setFontSize(9);
        doc.setFont("helvetica", "normal");
        doc.text(formatDate(flight.departureTime), midX - 30, startY + 11, { align: "center" });
        doc.text(`${flight.departureAirport.city}, ${flight.departureAirport.terminal}`, midX - 30, startY + 16, { align: "center" });
        ...
        doc.setFontSize(12);
        doc.setFont("helvetica", "bold");
        doc.text(flight.arrivalAirport.code, midX + 30, startY, { align: "center" });
        doc.text(formatTime(flight.arrivalTime), midX + 30, startY + 6, { align: "center" });
        doc.setFontSize(9);
        doc.setFont("helvetica", "normal");
        doc.text(formatDate(flight.arrivalTime), midX + 30, startY + 11, { align: "center" });
        doc.text(`${flight.arrivalAirport.city}, ${flight.arrivalAirport.terminal}`, midX + 30, startY + 16, { align: "center" });

        // Baggage Info Row
        doc.setFontSize(8);
        doc.setTextColor(100, 100, 100);
        doc.text(`Baggage: Cabin - 7Kg | Check-in - ${booking.baggage || '15kg'} | Class: ${passenger.flightClass}`, 20, 115);
        ...
        doc.setFontSize(9);
        doc.setFont("helvetica", "normal");
        doc.text(`Total Amount Paid: INR ${booking.totalAmount.toLocaleString()}`, 15, finalY + 23);
...
export const generateBoardingPassPDF = (booking) => {
    try {
        ...
        const flight = booking.flight;
        const passenger = booking.passenger;
        const passengerName = (passenger.name || `${passenger.lastName}/${passenger.firstName}`).toUpperCase();
        ...
        doc.setFontSize(9);
        doc.setTextColor(100, 100, 100);
        doc.text(`${flight.departureAirport.city} To ${flight.arrivalAirport.city.toUpperCase()} (${flight.arrivalAirport.terminal})`, 90, 20, { align: "right" });
        ...
        const boardingDate = new Date(new Date(flight.departureTime).getTime() - 45 * 60000);
        doc.text(formatTime(boardingDate.toISOString()).replace(" Hrs", ""), 55, row2Y);
        doc.setFontSize(8);
        ...
        doc.text("Gate is subject to change and will close 25 minutes prior to departure.", 8, 78);
        ...
        doc.setFontSize(10);
        doc.setFont("helvetica", "bold");
        doc.text(`${flight.departureAirport.code} -> ${flight.arrivalAirport.code}`, stubX, 22);
        ...
        doc.setTextColor(0, 0, 0);
        doc.text(formatDate(flight.departureTime), stubX + 25, 35, { align: "right" });
