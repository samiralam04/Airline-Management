...
const AIRPORT_COORDINATES = {
  DEL: { lat: 28.5562, lon: 77.1, name: "Delhi", country: "India" },
  ...
  GOI: { lat: 15.38, lon: 73.8314, name: "Goa (Dabolim)", country: "India" },
  COK: { lat: 10.1518, lon: 76.393, name: "Kochi", country: "India" },
  ...
  SXR: { lat: 34.0087, lon: 74.774, name: "Srinagar", country: "India" },
  TRV: {
    lat: 8.4821,
    lon: 76.92,
  ...
  PAT: { lat: 25.5912, lon: 85.088, name: "Patna", country: "India" },
  ...
  LHR: { lat: 51.47, lon: -0.4543, name: "London Heathrow", country: "UK" },
...
export const getDistance = (originCode, destinationCode) => {
  ...
  const seed =
    (originCode || "").charCodeAt(0) + (destinationCode || "").charCodeAt(0);
...
export const generateMockFlights = (origin, destination, date) => {
  ...
  const mockFlights = [];
  const seed =
    (origin || "A").charCodeAt(0) +
    (destination || "B").charCodeAt(0) +
  ...
  const isOriginIndia = originAirport
    ? originAirport.country === "India"
    : true;
  const isDestIndia = destinationAirport
    ? destinationAirport.country === "India"
    : true;
  ...
  if (isDomestic) {
    ...
    availableAirlines = AIRLINES.filter(
      (a) => a.type === "INTERNATIONAL" || ["AI", "UK", "6E"].includes(a.code),
    );
  }
  ...
  for (let i = 0; i < numFlights; i++) {
    ...
    const departureTime = new Date(
      `${date}T${hourOffset.toString().padStart(2, "0")}:${minuteOffset.toString().padStart(2, "0")}:00`,
    );
    const arrivalTime = new Date(
      departureTime.getTime() + durationTotalMinutes * 60 * 1000,
    );
    ...
    mockFlights.push({
      ...
      arrivalAirport: {
        ...
        city:
          AIRPORT_COORDINATES[destination]?.name ||
          destination ||
          "Destination",
      ...
      refundable: i % 2 === 0,
      aircraft:
        airline.code === "SG" || airline.code === "6E"
          ? "Boeing 737"
          : "Airbus A320neo",
...
export const getMockFlightById = (id) => {
  ...
  return {
    ...
    arrivalAirport: {
      code: destination || "BOM",
      city:
        AIRPORT_COORDINATES[destination]?.name || destination || "Destination",
      terminal: "T" + (1 + ((seed + 1) % 3)),
      coordinates: AIRPORT_COORDINATES[destination],
    },
    departureTime: departureTime.toISOString(),
